...
void execute_syntax_tree(ASTreeNode *node, t_env *env_list)
{
	...
	if (ft_strcmp(node->command, "|") == 0) {
        fprintf(stderr, "[DEBUG] Pipe detected in AST inside execute_syntax_tree().\n");
        handle_pipe(node, env_list);
        return;
    }

    // If it's a regular command, execute normally (we'll add this later)
    fprintf(stderr, "[DEBUG] Executing regular command: %s\n", node->command);
